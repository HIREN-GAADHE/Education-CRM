...
import {
    Dialog, DialogTitle, DialogContent, IconButton, Box, Typography, Avatar, Chip,
    Tabs, Tab, Grid, Card, CardContent, LinearProgress, Table, TableBody, TableCell,
    TableContainer, TableHead, TableRow, Divider, CircularProgress, Alert,
...
const StudentProfileDialog: React.FC<StudentProfileDialogProps> = ({ open, studentId, onClose }) => {
    ...
    const { data: profile, isLoading, error } = useGetStudentProfileQuery(studentId || '', {
        skip: !studentId,
    });
    ...
    const handleDownloadIdCard = async () => {
        ...
        try {
            await downloadFile(
                ...
                `id_card_${profile?.student.admission_number || studentId}.pdf`,
                token,
                tenant?.id
            );
        } catch (err) {
    ...
    const handleDownloadTranscript = async () => {
        ...
        try {
            await downloadFile(
                ...
                `transcript_${profile?.student.admission_number || studentId}.pdf`,
                token,
                tenant?.id
            );
        } catch (err) {
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth PaperProps={{ sx: { minHeight: '80vh' } }}>
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                            <Avatar
                                ...
                                sx={{ width: 100, height: 100, fontSize: 40, bgcolor: 'rgba(255,255,255,0.2)' }}
                            >
                                {profile.student.first_name[0]}{profile.student.last_name[0]}
                            </Avatar>
                            <Box>
                                <Typography variant="h4" fontWeight="bold">{profile.student.full_name}</Typography>
            ...
            <DialogContent sx={{ p: 0 }}>
                ...
                {error && (
                    <Alert severity="error" sx={{ m: 2 }}>Failed to load student profile</Alert>
                )}
