...
import {
    ...
    TableHead, TableRow, FormControl, InputLabel, Select, MenuItem, Snackbar, Tabs, Tab
} from '@mui/material';
import {
    Save as SaveIcon,
...
const AttendancePage: React.FC = () => {
    ...
    }, { skip: !selectedDate || !selectedClass });
        ...
        <Box>
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
                <Tabs value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)}>
                    <Tab label="Mark Attendance" icon={<EditIcon />} iconPosition="start" />
                    <Tab label="Attendance History" icon={<HistoryIcon />} iconPosition="start" />
                </Tabs>
            </Box>

            {/* Filters - Class and Date Selection (Always visible or shared? Maybe keep shared for class, date distinct) */}
            {/* Actually, let's keep Class selection common, but hide Date for History as it has its own range */}

            <Card sx={{ mb: 3, p: 2 }}>
                <Grid container spacing={2} alignItems="center">
                    <Grid item xs={12} sm={6} md={activeTab === 0 ? 4 : 8}>
            ...
            {activeTab === 0 ? (
                ...
                <AttendanceHistory selectedClass={selectedClass} />
            )}
        </Box>
    );
};
